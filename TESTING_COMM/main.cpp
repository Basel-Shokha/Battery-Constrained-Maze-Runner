...
int main() {
    ...
    svr.Post("/send_maze", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            // Compile steps into a clean text stream for the M5 Wi-Fi Client
            ...
            while (i < totalNodes - 1) {
                ...
                while (j < totalNodes - 1) {
                    ...
                    if (checkHeading == activeHeading) { gridsCount++; j++; }
                    else { break; }
                }

                int targetIntersectionR = path[j]["cell"][0].get<int>();
                int targetIntersectionC = path[j]["cell"][1].get<int>();
                int totalOpenGridsAhead = 0;
                int stopLimitRemainingGrids = 0;

                if (activeHeading == 0) {
                    totalOpenGridsAhead = currR; stopLimitRemainingGrids = targetIntersectionR;
                }
                else if (activeHeading == 2) {
                    totalOpenGridsAhead = (rows - 1) - currR; stopLimitRemainingGrids = (rows - 1) - targetIntersectionR;
                }
                else if (activeHeading == 1) {
                    totalOpenGridsAhead = (cols - 1) - currC; stopLimitRemainingGrids = (cols - 1) - targetIntersectionC;
                }
                else if (activeHeading == 3) {
                    totalOpenGridsAhead = currC; stopLimitRemainingGrids = targetIntersectionC;
                }

                stepBuffer << "STEP:" << totalCompressedSteps << "," << activeHeading << ","
                           << gridsCount << "," << totalOpenGridsAhead << "," << stopLimitRemainingGrids << "\n";
    ...
    svr.Get("/get_instructions", [&](const httplib::Request&, httplib::Response& res) {
        ...
        std::cout << "[WIFI LINK] M5 Stick downloaded mission array over Wi-Fi.\n";
    });
    ...
    svr.Post("/update_telemetry", [&](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            // NEW DIAGNOSTIC MONITOR PRINT: Confirms air packets are reaching the server
